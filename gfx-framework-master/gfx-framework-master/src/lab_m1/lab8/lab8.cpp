...
void Lab8::Init()
{
    ...
    // Light & material properties
    {
        ...
        point_light_pos[0] = glm::vec3(0, 1, 1);
        point_light_dir[0] = glm::vec3(0, -1, 0);

        point_light_pos[1] = glm::vec3(2, 1, 1);
...
void Lab8::Update(float deltaTimeSeconds)
{
    ...
    { // al doilea spot
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, point_light_pos[1]);
        modelMatrix = glm::scale(modelMatrix, glm::vec3(0.1f));
        RenderMesh(meshes["sphere"], shaders["Simple"], modelMatrix);
...
void Lab8::RenderSimpleMesh(Mesh* mesh, Shader* shader, const glm::mat4& modelMatrix, const glm::vec3& color)
{
    ...
    glUniform3fv(location, 2, glm::value_ptr(point_light_pos[0]));

    GLuint location1 = glGetUniformLocation(shader->program, "point_light_dir");
    glUniform3fv(location1, 2, glm::value_ptr(point_light_dir[0]));
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (window->KeyHold(GLFW_KEY_W)) {
            point_light_pos[0] -= forward * deltaTime * speed;
        }
        if (window->KeyHold(GLFW_KEY_A)) {
            point_light_pos[0] -= right * deltaTime * speed;
        }
        ...
        if (window->KeyHold(GLFW_KEY_E)) {
            point_light_pos[0] += up * deltaTime * speed; 
        }
        ...
        if (window->KeyHold(GLFW_KEY_1)) {
            point_light_pos[1] -= forward * deltaTime * speed / 2.0f;
        }
        if (window->KeyHold(GLFW_KEY_2)) {
            point_light_pos[1] -= right * deltaTime * speed / 2.0f;
        }
        ...
        if (window->KeyHold(GLFW_KEY_5)) {
            point_light_pos[1] += up * deltaTime * speed / 2.0f;
        }
        ...
        if (window->KeyHold(GLFW_KEY_UP)) {
            point_light_dir[0] -= forward * deltaTime * speed;
        }
        if (window->KeyHold(GLFW_KEY_RIGHT)) {
            point_light_dir[0] += right * deltaTime * speed;
        }
        ...
        if (window->KeyHold(GLFW_KEY_7)) {
            angle += glm::radians(10.0f) * deltaTime;
        }
