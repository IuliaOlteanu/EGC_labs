...
void main()
{
    ...
    vec3 world_normal = normalize(mat3(Model) * v_normal); // N
    ...
    float att_factor = 1.f / (dist * dist + 1);

    // TODO(student): Compute light
    color = object_color * (ambient_light + emisiva + att_factor * (diffuse_light + specular_light));
