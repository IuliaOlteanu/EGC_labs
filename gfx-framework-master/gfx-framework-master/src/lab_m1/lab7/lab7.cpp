...
{
    {
        ...
        RenderSimpleMesh(meshes["sphere"], shaders["LabShader"], modelMatrix, glm::vec3(0.25f, 0.45f, 0.75f));
    }

    {
        ...
        RenderSimpleMesh(meshes["box"], shaders["LabShader"], modelMatrix, glm::vec3(0.33f, 0.66f, 0.99f));
    }
...
void Lab7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    int location1 = glGetUniformLocation(shader->program, "light_position");
    glUniform3fv(location1, 1, glm::value_ptr(lightPosition));

    // uniform vec3 eye_position;
    glm::vec3 eyePosition = GetSceneCamera()->m_transform->GetWorldPosition();
    // TODO(student): Set eye position (camera position) uniform
    int location2 = glGetUniformLocation(shader->program, "eye_position");
    glUniform3fv(location2, 1, glm::value_ptr(eyePosition));
    ...
    int location4 = glGetUniformLocation(shader->program, "material_kd");
    glUniform1f(location4, materialKd);

    // ks
    // uniform float material_ks;
    int location5 = glGetUniformLocation(shader->program, "material_ks");
    glUniform1f(location5, materialKs);

    // object color
    // uniform vec3 object_color;
    int location6 = glGetUniformLocation(shader->program, "object_color");
    glUniform3fv(location6, 1, glm::value_ptr(color));
