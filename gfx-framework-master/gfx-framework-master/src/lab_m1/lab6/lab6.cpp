...
void Lab6::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix)
{
    ...
    // BONUS
    // TODO(student): Get shader location for uniform mat4 "Time"
    // trimit datele generale la shader prin variabile uniforme
    // shader->program = Id-ul programului shader
    // dupa ce avem locatia putem trimite la acest pointer inf 
    // timpul aplicatiei : Engine::GetElapsedTime()
    int location4 = glGetUniformLocation(shader->program, "Time");
    glUniform1f(location4, (GLfloat)Engine::GetElapsedTime());
